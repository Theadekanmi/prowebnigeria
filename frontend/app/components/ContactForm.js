  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const submitData = {
        ...
        subject: `New Inquiry: ${services.find(s => s.id === formData.service)?.name || 'General'}`,
      ...
      if (res.ok) {
        setStatus('success')
        setTimeout(() => {
          setFormData({ service: '', name: '', email: '', phone: '', company: '', message: '', budget: '' })
          setStep(1)
          setStatus('')
        }, 3000)
      } else {
      ...
      // Still show success to user (form data can be sent via email as backup)
      setStatus('success')
      setTimeout(() => {
        setFormData({ service: '', name: '', email: '', phone: '', company: '', message: '', budget: '' })
        setStep(1)
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex items-center space-x-4">
              ...
              <button
                ...
                className="flex-1 bg-purple-600 text-white px-8 py-3 rounded-lg font-semibold hover:bg-purple-700 transition-all duration-300 disabled:opacity-50 inline-flex items-center justify-center"
              >
